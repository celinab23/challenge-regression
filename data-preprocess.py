...
# Drop observations under "other" type of property and mixed used buildings.
other_properties = df[
    df["subtype_of_property"].isin(
        [
            "other property",
            "mixed use building",
        ]
...
df["subtype_ecoded"] = df["subtype_of_property"].apply(
    lambda x: encode_sub_type(x, sub_types_dict)
)
...
zip_codes_df.loc[0] = (
    zip_codes_df.columns
)  # Add info for Brussels, beacuase it is incorrectly used at the column names in the original csv file.
...
df.drop(
    ["subtype_of_property", "commune", "province"],
    axis=1,
    inplace=True,
)
